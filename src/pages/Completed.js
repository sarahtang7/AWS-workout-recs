...
const Completed = () => {
    ...
    useEffect(() => {
        async function fetchCompleted() {
          try {
            ...
            // POST request to get-completed lambda function
            if (userEmail) {
                const response = await fetch('https://zkeuos9g2a.execute-api.us-east-1.amazonaws.com/v1/completed', {
                    method: 'POST',
                    headers: {
    ...
    useEffect(() => {
        async function fetchWorkoutInfo() {
            try {
                for (const workoutId of completed) {
                    const response = await fetch('https://zkeuos9g2a.execute-api.us-east-1.amazonaws.com/v1/completed', {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json'
